...
import { getAvailableTrainers, assignAllInOneTrainers, type Trainer } from "@/actions/admin/admin.dashboard.action";
...
export function AllInOneTrainerModal({
  ...
  const fetchTrainers = async () => {
    setIsLoading(true);
    try {
      const fitnessTrainers = await getAvailableTrainers('FITNESS');
  ...
  const handleAssignTrainer = async () => {
    ...
    setIsAssigning(true);
    try {
      const result = await assignAllInOneTrainers(clientId, { fitnessTrainerId: selectedTrainerId });
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium">Select Fitness Trainer</label>
            {isLoading ? (
